namespace JiraLike.Application.Handler.Auth
{
    ...
    public class LoginUserHandler : IRequestHandler<LoginUserCommand, AuthResponseDto>
    {
        ...
        public LoginUserHandler(IRepository<UserEntity> repository, IPasswordHasher<UserEntity> passwordHasher,
        ...
        {
            var user = await _repository.FirstOrDefaultAsync(user => user.Email == request.LoginRequest.Email, cancellationToken)
                 ?? throw new EntityNotFoundException<UserEntity>(request);

            var verifyPassword = _passwordHasher.VerifyHashedPassword(user, user.PasswordHash, request.LoginRequest.Password);
            if (verifyPassword == PasswordVerificationResult.Success)
            {
                var accessToken = _tokenGeneratorService.GenerateAccessToken(request.LoginRequest.Email, user.IsActive,
                     user.Id);
